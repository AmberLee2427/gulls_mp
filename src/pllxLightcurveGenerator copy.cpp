    	...
    	if (Paramfile->identicalSequence && obsidx > 0)
	  	{
			...
			// Keep track of minimum impact parameter
			Event->umin=min(Event->umin,qAdd(tt,uu));
			
			// Transform to binary lens center of mass frame
			double xsCoM = tt * cosa - uu * sina + VBM_origin;
			double ysCenter = tt * sina + uu * cosa;

			// Store source position in lens frame (Einstein radii)
			Event->xs[idx] = xsCoM;
			...
			{
				double xs2CoM, ys2Center;  // second source position in CoM frame

				// Orbital motion frame correction?
				...
				// Compute magnification for second source
				...
				if (Paramfile->astrometry_on && astrom_ok) {
					// Map lens-frame (x1 along binary axis, x2 perpendicular) to sky (N,E)
					// Using standard rotation: N = x1*cos(φ_axis) - x2*sin(φ_axis); E = x1*sin(φ_axis) + x2*cos(φ_axis)
					double cN = cx_mas * cosPos - cy_mas * sinPos;   // North (mas)
					double cE = cx_mas * sinPos + cy_mas * cosPos;   // East (mas)
					Event->cNtrue[idx] = cN;
					Event->cEtrue[idx] = cE;
					// Lens-light blending for measured centroid (lens-centric NE)
					int filt2 = World[obsidx].filter;
					double m_src2 = Sources->mags[Event->source][filt2];
					double m_lns2 = Lenses->mags[Event->lens][filt2];
					double R_lens_rel2 = pow(10.0, -0.4 * (m_lns2 - m_src2));
					double F_src_tot_rel2 = Event->Atrue[idx];
					double f_blend2 = F_src_tot_rel2 / (F_src_tot_rel2 + R_lens_rel2);
					// initialize observed values here; noise added later in photometry
					Event->cNobs[idx] = f_blend2 * cN;
					Event->cEobs[idx] = f_blend2 * cE;
				} else {
				...
				// Proper rotation: N = x1*cos(φ_axis) - x2*sin(φ_axis); E = x1*sin(φ_axis) + x2*cos(φ_axis)
				if (Paramfile->astrometry_on && astrom_ok) 
				{
					double cN = cx_mas * cosPos - cy_mas * sinPos;   // North (mas)
					double cE = cx_mas * sinPos + cy_mas * cosPos;   // East (mas)
					...
					double F_src_tot_rel2 = Event->Atrue[idx];
					double f_blend2 = F_src_tot_rel2 / (F_src_tot_rel2 + R_lens_rel2);
					Event->cNobs[idx] = f_blend2 * cN;   // initialize observed (noise added in photometry)
					Event->cEobs[idx] = f_blend2 * cE;
				} else {
					Event->cNtrue[idx] = 0.0;
					Event->cEtrue[idx] = 0.0;
